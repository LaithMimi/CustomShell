...
//
//    if (sem_wait(sem) == -1) {
//        perror("sem_wait failed in saveToShm");
//        return;
//    }
//
//    shmData *matrix_op = (shmData *)target_addr;
//    memcpy(&(matrix_op->matrix), matrix, sizeof(Matrix));
//    strncpy(matrix_op->operation, operation, MAX_OP_LEN - 1);
//    matrix_op->operation[MAX_OP_LEN - 1] = '\0';
//
//    printf("Wrote to shared memory: operation=%s, rows=%d, cols=%d\n",
//           matrix_op->operation, matrix_op->matrix.rows, matrix_op->matrix.cols);
//
//    (*mat_counter)++;
//
//    if (sem_post(sem) == -1) {
//        perror("sem_post failed in saveToShm");
//    }
//}